...
import { lendleRepay } from "./actions/kit/lendle";
...
function LendleRepayForm() {
  ...
  const handleRepay = async () => {
    ...
    try {
      ...
      await showToast({ style: Toast.Style.Animated, title: "Repaying on Lendle..." });

      const result = await lendleRepay(agent, selectedToken as Address, amount, parseInt(rateMode) as 1 | 2);

      await showToast({ style: Toast.Style.Success, title: "Repay complete!" });
      push(
        <Detail
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action title="Repay" icon={Icon.Upload} onAction={handleRepay} />
        </ActionPanel>
      }
    >
      ...
      <Form.Dropdown id="rateMode" title="Rate Mode" value={rateMode} onChange={(v) => setRateMode(v as "1" | "2")}>
...
export default function LendleRepay() {
  return (
    <WalletProvider>
      <RequireWallet>
        <LendleRepayForm />
